	...
	switch($main_page)
	{
		...
		case "1":
				...
				if (isset ($_POST['edit_this']))
					{
						...
						switch($_POST['edit_this'])   //обработчик нажатия чекбокса
						{
								case "1":
								
								$edit_query_1 = mysqli_query($mysqli, "UPDATE `engine_room` SET `name_eng_room`= '$edit_1_2' WHERE `id_eng_room`= '$edit_1_1' ");
					
										if ($edit_query_1)
											{
												echo ' </br></br><h1 style="font-style:italic" align="center">Запись успешно изменена!</h1></br></br></br>
								...
								$edit_query_2 = mysqli_query($mysqli, "UPDATE `rack` SET `name_rack`= '$edit_2_3' WHERE `id_rack`= '$edit_2_1'");
								...
								$edit_query_3 = mysqli_query($mysqli, "UPDATE `unit` SET `name_unit`= '$edit_3_2' WHERE `id_unit`= '$edit_3_1' ");
								...
								$edit_query_4 = mysqli_query($mysqli, "UPDATE `server` SET `name_server`= '$edit_4_3' WHERE `id_server`= '$edit_4_1' ");
								$edit_query_4_1 = mysqli_query($mysqli, "UPDATE `server` SET `type_server`= '$edit_4_4' WHERE `id_server`= '$edit_4_1'");
					
										if ($edit_query_4 || $edit_query_4_1)
											{
												echo ' </br></br><h1 style="font-style:italic" align="center">Запись успешно изменена!</h1></br></br></br>
												<p style="text-align: center;"><input type="submit" onclick=location.href="edit.php" class = "default_button" value="Вернуться"></p></br>';
											}
				...
				else 
				{
					...
					echo '<p style="text-align: center;"><input type="submit" onclick=location.href="edit.php" value="Вернуться" class = "default_button"></p>';
				}	
